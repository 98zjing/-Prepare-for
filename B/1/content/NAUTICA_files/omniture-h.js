...
/********** 1st PARTY COOKIE SECTION ***************************************/
s.launchDate='';				//date of launch for migrating to first party cookie leave blank if not doing migration
s.trackingServer="metrics.nautica.com";
s.trackingServerSecure="smetrics.nautica.com";

/**************************** STANDARD CONFIG SECTION *********************/

s.campaignID='cid,camp';   //campaign variable that flows into the Tracking Code report
s.internalCID='ab';        //internal campaign ID
s.gmt='-5';                //hour difference from GMT for time parting set and client time zone
s.currencyCode="USD"       //currency code
...
/********GSI Method to Set Internal Filters ********************************/
...
/**********************************************************************************************/
	...
	if(!s.prop8 && s.internalCID)
		s.prop8=s.getQueryParam(s.internalCID);
}
